...
- [x] **ROOT CAUSE FOUND (2026-08-24): production runs nginx, which never reads `.htaccess`.** GSC's URL-level drilldown for "Not found (404)" (121 of 144 not-indexed URLs, 84%) showed `wedgfield.php`, `audubon-park.php`, and other pages that `.htaccess` *should* redirect — crawled as recently as 2026-08-21/22. Confirmed live with curl: `audubon-park.php` and every other `.htaccess` rule returns a bare 404/200 on production, not a redirect — the entire `.htaccess` redirect strategy (83 rules, going back to before this repo existed) has never actually taken effect. `orlando.php` in particular is **not** redirected — it's live at 200, still serving its old neighborhood list including a broken `dr-phillips.php` link (fixed → `doctor-phillips.php` in this session; real target file is `doctor-phillips.php`, not `dr-phillips.php`). A ready-to-install nginx equivalent already existed at [`nginx-redirects.conf`](nginx-redirects.conf) but was written 2026-08-09 and was never installed *and* never updated for the 17 pages pruned 2026-08-16 — brought it up to date (added the 17 pruned-page redirects + the `dr-phillips.php` fix) in this session. **Still needs to be installed on the server by SSH** (see install instructions at the top of that file) — nothing in this repo can do that part.
...
- [ ] **Average Position sitting at ~49, worst week yet is 2026-08-17 to 08-23 (impression-weighted 64.9, vs ~45-53/week prior).** GSC Performance export 2026-08-25, "Last 3 months," confirms this isn't noise settling out — it got worse right after the 08-16 pruning/homepage rewrite. Drivers, from the Pages/Queries breakdown: (1) the highest-impression pages rank deep — `residential.php` (66.86), `commercial.php` (51.14), `junk-removal-orlando.php` (74.35, brand new 08-15, expected to take time), `appliance-removal.php`/`carpet-removal.php`/`concrete-removal.php` all 45-53 — these outweigh the well-ranking homepage (16.77) in the average; (2) the actual money queries — "junk removal orlando fl" (pos 81.7), "junk removal orlando" (76.0), "orlando junk removal" (79.9), "junk hauling orlando" (77.1) — sit on page 8-9; (3) **the nginx redirect bug above is directly splitting ranking signal**: `estate_cleanout.php` (broken duplicate, unredirected) pulls 292 impressions @ pos 49.1 vs the correct `estate-cleanout.php` at only 47 impressions @ pos 57.6 for the same "estate clean-out orlando" query cluster — Google can't consolidate them while the redirect is dead. Expect the nginx fix to help this too, not just the 404 count; re-check position 2-4 weeks after it's installed. `residential.php`/`commercial.php` ranking poorly despite real volume is a content/on-page problem, not technical — next lever after the redirect fix lands.
