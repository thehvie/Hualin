...
- [x] **Monitor Coverage report over next 2-4 weeks — watch for "Crawled/Discovered – not indexed" to drop from 42 and indexed count to climb from 35.** GSC export 2026-08-24: indexed climbed 35 → 62 (+77%); "Crawled – not indexed" (15) + "Discovered – not indexed" (2) = 17, down from 42 (-60%). Pruning is working — keep watching, don't declare done yet.
- [x] **ROOT CAUSE FOUND & CORRECTED (2026-08-24/25): production runs nginx, which never reads `.htaccess`** — but only *part* of the redirect strategy was actually missing, not all of it. Initial diagnosis (08-24) wrongly claimed none of `.htaccess`'s 83 rules had ever taken effect, based on spot-checking only `audubon-park.php` and `orlando.php`. Full server inspection on 08-25 (`sudo cat /etc/nginx/sites-enabled/haulinjunkies.com`) found production already has a working, live redirect file at `/etc/nginx/snippets/haulinjunkies-redirects.conf`, included directly into the site's nginx server block — it already correctly 301s `apartment_cleanouts.php`, `estate_cleanout.php`, and all the baldwin-park-\*/winter-park-\*/windermere-\*/maitland-\* sub-pages (confirmed live with curl, all return 301). The `.html → .php` rewrite and `/index.php → /` redirect are also already live, directly in the server block. **What was actually missing**: the 17 location pages pruned 2026-08-16, `orlando.php → /`, and `dr-phillips.php → doctor-phillips.php` (real target file is `doctor-phillips.php`, not `dr-phillips.php` — also fixed the broken link in `orlando.php` itself). Gave the user an append-only patch (matching the live file's `location = /x { return 301 /y; }` syntax) to paste onto the end of `/etc/nginx/snippets/haulinjunkies-redirects.conf` via the DigitalOcean web console, then `sudo nginx -t && sudo systemctl reload nginx`. [`nginx-redirects.conf`](nginx-redirects.conf) in this repo is now just that addendum, corrected to match the real live mechanism. **Confirm after reload**: `curl -I` the 17 pruned URLs + `orlando.php` + `dr-phillips.php`, all should be 301 now.
...
- [ ] **Average Position sitting at ~49, worst week yet is 2026-08-17 to 08-23 (impression-weighted 64.9, vs ~45-53/week prior).** GSC Performance export 2026-08-25, "Last 3 months," confirms this isn't noise settling out — it got worse right after the 08-16 pruning/homepage rewrite. Drivers, from the Pages/Queries breakdown: (1) the highest-impression pages rank deep — `residential.php` (66.86), `commercial.php` (51.14), `junk-removal-orlando.php` (74.35, brand new 08-15, expected to take time), `appliance-removal.php`/`carpet-removal.php`/`concrete-removal.php` all 45-53 — these outweigh the well-ranking homepage (16.77) in the average; (2) the actual money queries — "junk removal orlando fl" (pos 81.7), "junk removal orlando" (76.0), "orlando junk removal" (79.9), "junk hauling orlando" (77.1) — sit on page 8-9; (3) `estate_cleanout.php` (old duplicate) pulls 292 impressions @ pos 49.1 vs the correct `estate-cleanout.php` at only 47 impressions @ pos 57.6 for the same "estate clean-out orlando" query cluster — **correction (08-25): this redirect is actually already live** (confirmed via curl, 301 working), so this split is Google's ranking/index data lagging behind a fix that predates this GSC pull, not an active bug — worth re-checking in a few weeks to confirm it consolidates, but don't chase it as a current cause. `residential.php`/`commercial.php` ranking poorly despite real volume is a content/on-page problem, not technical — that's the real next lever.
...
- [x] **Monitor Coverage report** in Search Console over next 2-4 weeks — indexed page count climbed from ~35 to 62 as of 2026-08-24. Keep watching another 2-4 weeks before calling the pruning fix complete; see the 404 note above for the next thing to chase.
